...
void GetFieldLen(SDK_8583_STFIELDDEF *pstFieldDef, s32 *iOutputFieldLen)
{
...
s32 UnpackIso8583(const u8 *pheSrc,s32 iSrcLen)
{
	s32 i = 0;
	s32 j = 0;
	s32 iCount = 0;
	...
	u8  buf8583Pkg[2048];
	u8  bufTmp2[2048];
	u8  printBuf[2048 + 100];
	SDK_8583_STFIELDDEF *pstTabIsoFields = TabIsoFields + 1;
	...
	for(i = 0;i < iCount;i++)
	{
		sprintf(bufTmp2 + i*3,"%02X ",buf8583Pkg[i]);
	}
	sprintf(printBuf,"ISO8583:=%s",bufTmp2);
	sdkPrintStr(printBuf, st_font, SDK_PRINT_LEFTALIGN, 0, 0);
	sprintf(printBuf,"TotalLen=0X%02X(%03d)",iCount,iCount);
	sdkPrintStr(printBuf, st_font, SDK_PRINT_LEFTALIGN, 0, 0);
	/*TPDU*/
	...
	for(i = 0;i < 5;i++)
	{
		sprintf(bufTmp2 + iBufLen + i * 3,"%02X ",(buf8583Pkg + iCurrentLen)[i]);
	}
	
	sdkPrintStr(bufTmp2, st_font, SDK_PRINT_LEFTALIGN, 0, 0);
	...
	for(i = 1;i < 64;i++)
	{
	   if(buf[i/8] & (0x80>>i%8))
	   {
		 ...
		 if(pstTabIsoFields[i].nFLenLen == 0)
		 {
		 	iFieldLen = pstTabIsoFields[i].nFDataLen;
			sprintf(bufTmp2,"[%02d]<%03d>:",i+1,iFieldLen);
			iBufLen = strlen(bufTmp2);
			GetFieldLen(&pstTabIsoFields[i],&iFieldLen);
			for(j = 0;j < iFieldLen;j++)
			{
				sprintf(bufTmp2 + iBufLen + j * 3,"%02X ",(buf8583Pkg + iCurrentLen)[j]);
			}
		 	iCurrentLen += iFieldLen;
		 }
		 else if(pstTabIsoFields[i].nFLenLen == 2)
		 {
		 	iFieldLen = ((buf8583Pkg + iCurrentLen)[0] & 0x0F) 
							+ ((buf8583Pkg + iCurrentLen)[0] >> 4) * 10;
			if(iFieldLen > pstTabIsoFields[i].nFDataLen)
			{
		...
		else if(pstTabIsoFields[i].nFLenLen == 3)
		 {
	...
	sdkPrintStr("pack ok", st_font, SDK_PRINT_LEFTALIGN, 0, 0);
	return SDK_OK;
}
